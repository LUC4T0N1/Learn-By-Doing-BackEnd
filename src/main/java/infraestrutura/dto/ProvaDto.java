...
@Schema(name = "Provas", description = "JSON com informações sobre uma prova específica")
public class ProvaDto {
  ...
  public static ProvaDto instanciarPorEntidade(
      Prova prova,
      List<QuestaoRespondida> respostas,
      Boolean setResposta,
      ...
      for (QuestaoRespondida questao : respostas) {
        questaoDtos.add(
            QuestaoDto.instanciar(
                questao.getQuestao(),
                questao.getRespostaAluno(),
                questao.getComentarioProfessor(),
                questao.getNotaAluno(),
                questao.getId(),
                setResposta,
                valores.stream()
                    .filter(v -> v.getQuestao().equals(questao.getQuestao().getId()))
      ...
      for (Questao questao : prova.getQuestoes()) {
        questaoDtos.add(
            QuestaoDto.instanciar(
                ...
                setResposta,
                valores.stream()
                    .filter(v -> v.getQuestao().equals(questao.getId()))
                    .collect(Collectors.toList())
                    .get(0)
                    .getValor(),
                fazer));
